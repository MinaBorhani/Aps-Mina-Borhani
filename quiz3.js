  ...
  */
  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
      // Complete here
      return restaurants.filter((restaurant) => restaurant, numberOfPeople).map((restaurant) => restaurant.name);
      },
    findRestaurantServingDish: function (dishName) {
      // Complete here
      return restaurants.filter((serveDish)=>serveDish,dishName).map(serveDish=>serveDish.name);
    },
    countNumberOfRestaurantsInArea: function (area) {
      // Complete here
      return restaurants.filter(Restaurants=>Restaurants.address.area===area).length;
    },
  }
